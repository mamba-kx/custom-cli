  ...
  <div class="homeWrapper">
    <div class="headSection">
      ...
      <div class="convertCenter">
        <div
          class="convert"
          v-for="item in homeIconData?.convertCenterList"
          :key="item.name"
        >
          <img
            class="icon"
    ...
    <div class="allActivity">
      <ul class="bannerList">
        <li v-for="item in homeIconData?.hotActivitiesData" :key="item.id">
...
import { homeIcon } from '@/mock/index';
import { IHomeIcon, IConvertCenterList } from '@/interface/index';
export default defineComponent({
  setup() {
    const homeIconData: IHomeIcon = {
      convertCenterList: [],
      hotActivitiesData: []
    };
    let data = reactive({ homeIconData });
    // 模拟接口请求
    setTimeout(() => {
      data.homeIconData = homeIcon;
    }, 200);

    const router = useRouter();

    // 跳转兑换中心
    const skipConvertCenter = (item: IConvertCenterList) => {
      if (item.name === '兑换中心') {
    ...
    // 跳转定制活动
    const skipToH5 = (item: IConvertCenterList) => {
      window.location.href = item?.url || '';
    };
